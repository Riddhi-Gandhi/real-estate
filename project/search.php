...
if(isset($_POST['h_search'])){
   ...
   $h_available_date = $_POST['h_available_date'];
   $h_available_date = filter_var($h_available_date, FILTER_SANITIZE_STRING);
   $h_type = $_POST['h_type'];
   ...
   $select_properties = $conn->prepare("SELECT * FROM `property` WHERE Location LIKE '%{$h_location}%' AND date LIKE '%{$h_available_date}%' AND type LIKE '%{$h_type}%' AND offer LIKE '%{$h_offer}%' AND price BETWEEN $h_min AND $h_max ORDER BY date DESC");
   ...
   $select_properties = $conn->prepare("SELECT * FROM `property` WHERE Location LIKE '%{$location}%' AND type LIKE '%{$type}%' AND offer LIKE '%{$offer}%' AND bhk LIKE '%{$bhk}%' AND status LIKE '%{$status}%' AND furnished LIKE '%{$furnished}%' AND price BETWEEN $min AND $max ORDER BY date DESC");
   $select_properties->execute();
